...
function startQuizBySet(s, e, n) { 
    // s = จุดเริ่มต้น, e = จุดสิ้นสุด, n = ชุดที่
    if (!currentTopicQuestions || currentTopicQuestions.length === 0) {
        return alert("เกิดข้อผิดพลาด: ไม่พบข้อมูลข้อสอบในชุดนี้");
    }

    currentQuiz = currentTopicQuestions.slice(s, e); 
    currentQuizIdentifier = selectedExamTopic + '_ชุดที่_' + n; 
    
    // ตั้งชื่อหัวข้อที่จะโชว์ในหน้าทำข้อสอบ
    let displayTitle = selectedExamTopic + (currentActionMode === 'drill' ? ' (โหมดตะลุยชุดที่ ' + n + ')' : ' ชุดที่ ' + n);
    
    // เริ่มหน้าจอ UI ทำข้อสอบ
    startQuizUI(displayTitle, false); 
}
...
async function loadData() {
    try {
        // 1. โหลดไฟล์สำหรับ "จำลองสอบ" (mock_pool.json) จากโฟลเดอร์ data
        const response = await fetch('data/mock_pool.json'); 
        const data = await response.json();
        // เก็บไว้ใน allData หรือตัวแปรสำรองเพื่อใช้ทำ Mock/Trial
        allData = data.map(r => [
            r.question || "",
        ...
        console.log("✅ โหลดข้อมูลพื้นฐานสำเร็จ (สแตนบายรอโหลดไฟล์แยกหมวด)");
        
        // 2. อัปเดตความก้าวหน้าโดยรวม (Progress)
        // ฟังก์ชันนี้ต้องใช้ Logic ใหม่ที่ไม่ได้นับจาก allData ทั้งหมด (ตามที่แนะนำไปก่อนหน้า)
        if(currentUsername) updateOverallProgress();

    } catch (error) {
        // กรณีที่ไฟล์ mock_pool.json ยังไม่มี ไม่ต้องตกใจ ให้ Log ไว้เฉยๆ
        console.warn("⚠️ ไม่พบไฟล์ mock_pool.json ระบบจะโหลดไฟล์แยกตามวิชาแทนเมื่อมีการเลือกหมวด");
        
        // ถ้าไม่มีข้อมูลเลย ให้ล้าง allData เป็นค่าว่าง
        allData = [];
        if(currentUsername) updateOverallProgress();
    }
}
...
function updateOverallProgress() {
    // กำหนดจำนวนข้อสอบรวมทั้งหมดที่คุณมี (เช่น 1500 ข้อ)
    const TOTAL_QUESTIONS_IN_SYSTEM = 1500; 
    let totalDoneCount = 0;

    // นับจากคะแนนที่บันทึกไว้ใน completedScores
    for (let id in completedScores) {
        let s = completedScores[id];
        totalDoneCount += s.total || 0;
    }

    let overallPercent = Math.round((totalDoneCount / TOTAL_QUESTIONS_IN_SYSTEM) * 100) || 0;
    if(overallPercent > 100) overallPercent = 100; // กันบัค

    const progressBar = document.getElementById('overall-progress-bar');
    const progressText = document.getElementById('overall-progress-text');
    
    if(progressBar) progressBar.style.width = overallPercent + '%';
    if(progressText) progressText.innerText = overallPercent + '%';
}
...
async function showExamSets(mode) {
    ...
    // 🗺️ 1. สร้าง Map เชื่อมโยงชื่อหมวดกับชื่อไฟล์
    const fileMap = {
        "พ.ร.บ. ระเบียบข้าราชการฝ่ายอัยการ": "law_prosecutor_staff.json",
        "พ.ร.บ. องค์กรอัยการและพนักงานอัยการ": "law_prosecutor_org.json",
        "สำนักงานอัยการสูงสุด": "prosecutor_office.json",
        "พ.ร.บ. คุ้มครองข้อมูลส่วนบุคคล": "pdpa.json",
        "พ.ร.บ. ข้อมูลข่าวสารของราชการ": "official_info.json",
        "ระเบียบงานสารบรรณ": "saraban.json",
        "พ.ร.บ. ระเบียบข้าราชการพลเรือน": "civil_servant.json",
        "ความรู้เกี่ยวกับคอมพิวเตอร์": "computer_basic.json",
        "การใช้โปรแกรมในการปฏิบัติงาน": "software_usage.json",
        "ข้อสอบเสมือนจริงปี 2567": "real_ly.json"
    };

    const fileName = fileMap[selectedExamTopic.trim()];
    
    if (!fileName) {
        return alert("❌ ไม่พบไฟล์ข้อมูลสำหรับหมวด: " + selectedExamTopic);
    }

    showLoader('กำลังโหลดข้อสอบหมวด ' + selectedExamTopic + '...');

    try {
        // 📥 2. Fetch ไฟล์เฉพาะหมวดนั้นๆ (สมมติไฟล์อยู่ในโฟลเดอร์ data)
        ...
        // 🔄 3. แปลงข้อมูลใส่ currentTopicQuestions
        currentTopicQuestions = data.map(r => [
            r.question || "", 
            r.options[0] || "", 
            r.options[1] || "", 
            r.options[2] || "", 
            r.options[3] || "",
            (r.answer || "1").toString() + (r.explanation ? " " + r.explanation : ""),
            r.category || selectedExamTopic
        ]);

        // 📊 4. คำนวณชุดข้อสอบ (Logic เดิมของคุณ)
        ...
        hideLoader();
        alert("⚠️ โหลดข้อสอบไม่สำเร็จ: อาจเป็นเพราะยังไม่มีไฟล์ " + fileName + " บน GitHub ครับ");
    }
}

async function startMockExam() { 
    showLoader('กำลังเตรียมข้อสอบจำลอง 40 ข้อ...');
    try {
        // ดึงจากไฟล์รวมข้อสอบสำหรับ Mock โดยเฉพาะ
        const response = await fetch('data/mock_pool.json');
        const data = await response.json();
        
        let validData = data.map(r => [
            r.question || "", 
            r.options[0] || "", r.options[1] || "", r.options[2] || "", r.options[3] || "",
            (r.answer || "1").toString() + (r.explanation ? " " + r.explanation : ""),
            r.category || ""
        ...
        hideLoader();
        alert("⚠️ โหลดข้อสอบจำลองไม่สำเร็จ กรุณาเช็คไฟล์ mock_pool.json");
    }
}

async function startFreeTrial() {
    showLoader('กำลังเตรียมข้อสอบทดลอง...');
    try {
        const response = await fetch('./data/trial.json'); // ไฟล์เล็กๆ 10 ข้อ
        const data = await response.json();
        
        currentQuiz = data.map(r => [
            r.question || "", 
            r.options[0] || "", r.options[1] || "", r.options[2] || "", r.options[3] || "",
            (r.answer || "1").toString() + (r.explanation ? " " + r.explanation : ""),
            r.category || "ทดลองทำ"
        ...
        hideLoader();
        alert("โหลดข้อสอบทดลองไม่สำเร็จ");
    }
}
...
async function showExamSets(mode) {
    ...
    // 🗺️ 1. ต้องประกาศ fileMap และ fileName ก่อน (จุดที่ขาดไป)
    const fileMap = {
        "พ.ร.บ. ระเบียบข้าราชการฝ่ายอัยการ": "law_prosecutor_staff.json",
        "พ.ร.บ. องค์กรอัยการและพนักงานอัยการ": "law_prosecutor_org.json",
        "สำนักงานอัยการสูงสุด": "prosecutor_office.json",
        ...
        "ความรู้เกี่ยวกับคอมพิวเตอร์": "computer_basic.json",
        "การใช้โปรแกรมในการปฏิบัติงาน": "software_usage.json",
        "ข้อสอบเสมือนจริงปี 2567": "real_ly.json" // ตรวจสอบชื่อไฟล์ในเครื่องให้ตรงกับตรงนี้
    };
    ...
    try {
        // 📥 2. Fetch ข้อมูลจากโฟลเดอร์ data
        const response = await fetch(`./data/${fileName}`);
        if (!response.ok) throw new Error('ไม่สามารถโหลดไฟล์ ' + fileName + ' ได้');
        
        const data = await response.json();

        // 🔄 3. แปลงข้อมูล JSON เข้าสู่ระบบแอป
        currentTopicQuestions = data.map(r => [
            r.question || "", 
        ...
        // 📊 4. คำนวณชุดข้อสอบ (ชุดละ 25 ข้อ)
        let totalQ = currentTopicQuestions.length;
        let doneQ = 0;
        let h = '';
        let sets = Math.ceil(totalQ / 25);

        // กรณีไม่มีข้อสอบในไฟล์เลย
        if (totalQ === 0) {
            h = '<p style="text-align:center; padding:20px; color:red;">⚠️ ยังไม่มีข้อสอบในหมวดนี้</p>';
        } else {
            for (let i = 0; i < sets; i++) {
                let id = selectedExamTopic + '_ชุดที่_' + (i + 1);
                let isDone = (typeof isQuizCompleted === 'function') ? isQuizCompleted(id) : false;
                
                if (isDone) {
                    doneQ += (i === sets - 1 && totalQ % 25 !== 0) ? (totalQ % 25) : 25;
                }
        ...
        // 📈 5. อัปเดต UI Progress และหัวข้อ
        let p = totalQ > 0 ? Math.round((doneQ / totalQ) * 100) : 0;
        ...
        const setContainer = document.getElementById('set-buttons-container');
        const titleText = document.getElementById('exam-set-title');

        if (progressBar) progressBar.style.width = p + '%';
        if (progressText) progressText.innerText = `ทำหมวดนี้ไปแล้ว ${p}%`;
        if (titleText) titleText.innerText = (mode === 'drill' ? 'ตะลุยข้อสอบ: ' : '') + selectedExamTopic;
        if (setContainer) setContainer.innerHTML = h;
        ...
        console.error("Fetch Error:", error);
        hideLoader();
        alert("ขออภัย! ไม่สามารถโหลดข้อสอบได้ในขณะนี้ กรุณาลองใหม่อีกครั้ง");
    }
}
